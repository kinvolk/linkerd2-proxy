...
pub fn force_id_from_header<B, K>(req: &http::Request<B>, header: K) -> Option<identity::Name>
where
    K: AsHeaderName,
{
...
fn header_value_from_request<B, K, F, T>(req: &http::Request<B>, header: K, foo: F) -> Option<T>
where
    K: AsHeaderName,
    F: FnOnce(&str) -> Option<T>,
{
    req.headers().get(header).and_then(|value| {
        value
            .to_str()
            .ok()
            .and_then(|s| if s.is_empty() { None } else { foo(s) })
    })
}
